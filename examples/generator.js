...
// Simple SHA256 implementation for web workers
...
async function generateBIP39Mnemonic(privateKey) {
  try {
    const entropy = hexToBytes(privateKey);
    ...
    for (let i = 0; i < wordCount; i++) {
      ...
      if (isNaN(wordIndex) || wordIndex >= BIP39_WORDS.length) {
        throw new Error(`Invalid word index: ${wordIndex}`);
      }
      
      mnemonic.push(BIP39_WORDS[wordIndex]);
    }
...
onmessage = async (e) => {
    ...
    while(!found) {
      ...
      if(isMatched(prefix, npub, isPrefix)) {
        ...
        // generate mnemonic
        mnemonic = await generateBIP39Mnemonic(privatekey);
        
        // send the keys back to the main thread
        postMessage({ npub, nsec, publickey, privatekey, mnemonic, time });
      }
    }

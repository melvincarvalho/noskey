...
function generateBIP86Keys(mnemonic) {
  try {
    ...
    // Derive BIP86 path: m/86'/1'/0' (testnet, account 0)
    const account = root.derivePath("m/86'/1'/0'")
    
    // Get extended keys
    const tprv = account.toBase58()  // Private extended key
    const tpub = account.neutered().toBase58()  // Public extended key
    
    // Generate P2TR address from first receiving address (m/86'/1'/0'/0/0)
    const child = account.derive(0).derive(0)  // 0/0 = first receiving address
